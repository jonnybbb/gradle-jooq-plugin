...
jooq {
   ...
   normalization { config ->
      jdbc.url = config.jdbc.url.replaceAll('\\\\d','')      
      jdbc.user = 'ignore for up to date check'
      jdbc.driver = 'ignore for up to date check'
      generator.database.name = 'ignore for up to date check'     
   }
   sample(sourceSets.main) {
       jdbc {
           driver = '$jdbcDriver'
